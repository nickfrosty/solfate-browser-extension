import { routeRequest } from "../../utils/redirect";

const urls = [
  "http://*.sol/*",
  "https://*.sol/*",
  // not currently supported url schemes
  // "sol://*.sol/*",
  // "sol://*/*",
  "*://search.brave.com/search?q=*.sol&*",
  "*://duckduckgo.com/?q=*.sol&*",
  "*://duckduckgo.com/?q=*.sol%2F*",
  "*://www.bing.com/search?q=*.sol*",
...
/**
 * When this listener is active, the action popup button will open the extension in a new tab for browsing
 */
...
/**
 *  Listen for forwarded requests from the front end "content" script
 */
chrome.runtime.onMessage.addListener(async function (
  ...
  if (request?.url) await routeRequest(request.url);
  // else if (request?.solanaUrl) await redirect(request.solanaUrl);
  else {
    const error = "Unknown message request";
    // console.warn("[SOLFATE] ", error);
    sendResponse({ error });
  }
});

/**
 * Listen for web request errors, like when DNS does not resolve
 *
 * NOTE: This is useful if a browser tried to resolve the `.sol` TLD (which most don't)
 */
chrome?.webRequest?.onErrorOccurred?.addListener(
  async (details) => {
    if (details.error === "net::ERR_NAME_NOT_RESOLVED") {
      // console.log("[SOLFATE] ", "webRequest?.onErrorOccurred ", details.url);

      await routeRequest(details.url);
      return { cancel: true };
    }
    // else console.log("[SOLFATE] ", "unhandled error: ", details.error);
  },
  {
    urls: ["http://*.sol/*", "https://*.sol/*"],
  },
);

/**
 * Listener for requests that are auto-converted into search engine queries by the browser.
 * Since .sol is not a recognized domain extension, many browsers will automatically search for the domain in
 * the user's default search engine if entered into the address bar. This listener redirects the search request.
 *
 * Supported search engines: Google, DuckDuckGo, Brave, Bing
 */
chrome?.webRequest?.onBeforeRequest?.addListener(
  async (details) => {
    const solanaUrl = new URL(details.url).searchParams.get("q");
    // console.log("[SOLFATE] ", "webRequest?.onBeforeRequest ", solanaUrl);

    await routeRequest(solanaUrl);
...
chrome?.webNavigation?.onBeforeNavigate?.addListener(
  async (details) => {
    const solanaUrl = new URL(details.url).searchParams.get("q");
    console.log("[SOLFATE] ", "webNavigation onBeforeNavigate:", solanaUrl);
